...
echo 'hi';
           ...
           (select statecode from tbl_state_codes where idno=p.Source_of_destination) as Source_of_destination,v.Supplier_Name,c.Currency,it.Seller_SKU,l.LocationName from Purchase_invoice_line_items i left join  Purchase_invoice_header p on p.idno = i.Bill_number left join suppliers v on v.idno = p.Vendor_name left join tbl_currency c on c.CurrencyId = p.Currency left join Item_master it on it.idno = i.itemid left JOIN tbl_location l on l.LocationId = p.`Location` where p.Statusbit = 2 and i.itemid != ''";
           if($_GET["type"] == "single"){
